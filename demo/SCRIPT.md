...
| 0:55 | `demo/stills/leg-a-referee.png` — `<A>/8`, failing test name visible | "The hidden referee suite, hashed before either run: `<A>` of 8." |
...
| 1:57 | New task, built-in Agent mode, the authorised-smoke `pytest` prompt → `RATCHET blocked execute_command …` | "Now a deliberate bypass of the instruction layer. The hook catches what gets past the rules, and that block is a ledger line." |
| 2:05 | Terminal: `python -m rx report` — the BLOCK line | (let it read) |
| 2:08 | Bob, `4 - Ratchet Review`, the `Spawn the code-reviewer subagent …` prompt → findings table, `VERDICT:` | "Review is read-only: one spawned persona, two more passes Bob runs itself, a verdict. Every subagent call went through the same hook." |
...
| 2:19 | Terminal: `rx verify` PASS → one-byte tamper → FAIL naming the line → restore → PASS | "The ledger is HMAC-chained and gap-checked by a script with no model in it. Tamper-evident, not tamper-proof — a person with a shell can still do anything." |
...
- Never say: "physically impossible", "cannot widen", "tamper-proof", "append-only", "fan-out", or anything implying the ledger blocks a call live (spec §2.3).
